...
interface CreateCaseResponse { success: boolean; data: PatientCase }

const PAGE_TITLE: Record<string, string> = {
  law_firm: 'Cases',
  provider: 'Assigned Cases',
  funder:   'Assigned Cases',
  admin:    'All Cases',
}
...
export default function CasesPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      const res = await api.post<CreateCaseResponse>('/api/cases', {
        patient_name: patientName, case_number: caseNumber,
      })
      setCases(prev => [res.data, ...prev])
      setPatientName(''); setCaseNumber(''); setShowForm(false)
    } catch (err) {
  ...
  const title = user ? (PAGE_TITLE[user.role] ?? 'Cases') : 'Cases'
      ...
      <main className="p-8">
        ...
        {showForm && (
          <form onSubmit={handleCreate} className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
            ...
            {formError && (
              <p className="mt-3 text-sm text-red-600 bg-red-50 border border-red-200 px-3 py-2 rounded-lg">{formError}</p>
            )}
        ...
        {loading ? (
          ...
          <CaseTable cases={cases} userRole={user?.role} />
        )}
      </main>
    </AppShell>
  )
